...
const Profile = () => {
 ...
 const fetchUser = async() =>{
  ...
  const deposit = async (e)=>{
    e.preventDefault();
    await axios.post('http://localhost:6001/deposit', {'userId': userId, "depositAmount": depositAmount, "depositMode":depositMode}).then(
      (response)=>{
  ...
  const withdraw = async (e)=>{
    e.preventDefault();
      await axios.post('http://localhost:6001/withdraw', {"userId": userId, "withdrawAmount":withdrawAmount, "withdrawMode":withdrawMode}).then(
        (response)=>{
          localStorage.setItem('balance', response.data.balance);
          fetchTransactions();
          setActionType('Transactions');
